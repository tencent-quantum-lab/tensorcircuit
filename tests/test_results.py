...
def test_readout():
    ...
    def run(cs, shots):
        ...
        ts = []
        for c in cs:
            count=c.sample(batch = shots,allow_state=True, readout_error=readout_error,format='count_dict_bin')
            ts.append(count)
        return ts
    ...
    mit_count1 = mit.apply_correction(raw_count, [1,3,2],method = 'inverse')  #  direct(Max2),iterative(Max3), inverse,square
